...
# Examples:
#   ./make-livehtml
#   ./make-livehtml -p 9000
#   ./make-livehtml --port 9000
# ------------------------------------------------------------------------------
...
function main() {
    ...
    local sphinx_docker_port=$SPHINX_DOCKER_PORT
    ...
    while [[ $options_index -le $# ]]; do
        ...
        case "${!options_index}" in
            -p|--port)
                ...
                sphinx_docker_port="${!options_index}"
                ;;
            -h|--help)
                help
                exit 0
    ...
    if [[ -z "$sphinx_docker_port" ]]; then
        fatal "The port cannot be empty. Please use '-p' or '--port'."
    fi
    ...
    docker run --rm -it \
        -e USER_ID=$UID \
        -p $sphinx_docker_port:$sphinx_docker_port \
        -v $PROJECT_DIR:/doc \
        $sphinx_docker_image \
            make SPHINXPORT=$sphinx_docker_port livehtml
}
...
function help() {
    ...
    printf "  -p, --port PORT  The port on the host machine Sphinx is bound to\n"
    printf "                   The default port is $SPHINX_DOCKER_PORT\n"
    printf "  -h, --help       Print this message and exit\n"
    printf "\n"
    printf "Examples:\n"
    printf "  ./make-livehtml\n"
    printf "  ./make-livehtml -p 9000\n"
    printf "  ./make-livehtml --port 9000\n"
}
